...
void MainWindow::saveFile(){
  ...
  if(!savePath.isEmpty()){
      ...
      filePath=savePath;
      this->setWindowTitle(tr("X0 Compiler - ")+fileName);
    }
}
...
//    process.start("bash", QStringList() << "-c" << QString(tr("g++ ")+filePath+tr(" -o ")+buildPath+tr(";")+buildPath));
    QString content = ui->editor->document()->toPlainText();
    string s = content.toStdString();
    int temp = Excute(s);
    ui->outputText->appendPlainText(res);
    ui->outputText->appendPlainText("process exit...with code "+QString::number(temp));
...
void MainWindow::about(){
  QMessageBox::information(this,tr("关于"),tr(" X0-Compiler v1.0 \n"),QMessageBox::Ok);
}
